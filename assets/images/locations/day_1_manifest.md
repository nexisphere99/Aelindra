...
file (Aldric, Ash, Ilara, Nara, Vael, Marta), so there's no
`npcs/day_1_manifest.md`; this is the only Day 1 art file. Ash's portrait
resolves via `$bardPortraitSlug` (`ash_male.png`/`ash_female.png`)   see
...
routes share (waking, the mirror, Ash's visit, the first night) plus
