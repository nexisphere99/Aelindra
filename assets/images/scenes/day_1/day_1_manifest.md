...
| `day1ft_ash_male.png` | day1_ash_futa scene banner (male Ash / default   supersedes day1ft_ash_paradox.png, same scene gender-branched to match day1_ash.tw's pattern) | Two figures in a dim tower room. A half-elf bard (male presentation) with honey eyes sits cross-legged on the floor, lute in lap, looking up with an expression of tender recognition. On the bed above, a young woman in a white nightgown sits with knees drawn up, dark hair falling around her face, expression guarded but cracking toward trust. Candlelight. Warm shadows. Atmosphere: the first person to hear the truth, choosing fascination over judgment, a friend who understands between. |
| `day1ft_ash_female.png` | day1_ash_futa scene banner, only if the player chose a female Ash | Same scene, same composition, same candlelight and warm shadows   the bard on the floor is the female presentation instead: sharper/angular features, dark copper-toned hair, honey-coloured eyes, pointed ears, same lute. Everything else identical to day1ft_ash_male.png. |
...
The Ash-visit banner is gender-branched the same way as the female
route's: `<<set _ashVisitImg to ($bardGender is "female") ?
"day_1/day1ft_ash_female" : "day_1/day1ft_ash_male">>` then
`<<sceneImage _ashVisitImg "Ash's visit">>` in `day1_ash_futa.tw`. This
replaced an earlier ungendered `day1ft_ash_paradox.png` that predates
the Ash gender-choice system   that file is now unused; delete it from
`day_1/` next time this folder gets cleaned up. Every other Day 1 futa
image is a plain `<<sceneImage "day_1/slug" "...">>` call with no
branching (the choice that branches those is `$gamePath`, decided back
on Day 0, not `$bardGender`).
